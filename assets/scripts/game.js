      ...
      $(document).on("click", "button.character", function() {
        ...
        if (!$(this).hasClass("current")) {
          ...
          if (yourPlayerPickComplete === false ) {
            ...
            $("#result").text("You've selected " + characters[currentCharacter].name + ".");
          }
            ...
            $("#result").text(characters[currentCharacter].name + " will start a fight with " + characters[defenderCharacter].name + ". Click attack to try and murder " + characters[defenderCharacter].name + ".");
          }
      ...
      $(document).on("click", "#attackBTN", function() {
        ...
        if (enemyHP <= 0) {
          if ($("#availableEnemies").is(':empty')) {
            $("#result").text("You have defeated " + characters[defenderCharacter].name + ". The princess is at another castle.");
            $("#attackBTN").addClass("hidden");
          }
          else {
            $("#result").text("You have defeated " + characters[defenderCharacter].name + ". Select a new enemy to attack.");
          }
          ...
          if (currentHP <= 0) {
            $("#result").text(characters[defenderCharacter].name + " has defeated " + characters[currentCharacter].name + ". You loose.");
            $("#attackBTN").addClass("hidden");
          } 
          else{
            $("#result").text( characters[currentCharacter].name + " has attacked " + characters[defenderCharacter].name + " for " + currentAttackPower + ". " + characters[defenderCharacter].name + " has attacked back with " + enemyAttackPower + "." );
          }
        } 
      });
    });